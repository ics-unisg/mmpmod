...
The vision model uses a YOLOv11-based segmentation and scene-understanding pipeline, trained on a phlebotomy dataset developed within the ProAmbitIon research project, to interpret visual context relevant for disambiguation. The model is found at `mmpmod/ambiguityresolver/ml/YOLO-model/best.pt`.
...
* [Optional] In case of multiple connected cameras, specify which one to use by setting the respective identifier in `mmpmod/ambiguityresolver/camera/camera.py` (line `camera = cv2.VideoCapture(0)`).
...
The process was enacted 50 times while being monitored by a set of sensors producing IoT data; these data was abstracted into a stream of process events by activity detection services. This stream, which includes ambiguous process events, was used as the input for the prototype.
